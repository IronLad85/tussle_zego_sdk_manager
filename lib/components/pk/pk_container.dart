...
class _ZegoPKContainerViewState extends State<ZegoPKContainerView> {
  ...
  Widget audiencePKView() {
    return ValueListenableBuilder<List<PKUser>>(
        ...
        builder: (context, pkusers, _) {
          final pkAcceptUser =
              pkusers.where((element) => element.hasAccepted).toList();
          return PKMixerView(
  ...
  List<Positioned> hostPKViews(
      List<PKUser> pkUsers, BoxConstraints constraints) {
    final views = <Positioned>[];
    for (final pkuser
        in pkUsers.where((element) => element.hasAccepted).toList()) {
      ...
      final positioned = Positioned(
          ...
          child: Container(
            width: newRect.right - newRect.left,
            height: newRect.bottom - newRect.top,
            color: pkuser.userID == ZEGOSDKManager().currentUser!.userID
                ? Colors.yellow
                : Colors.blue,
