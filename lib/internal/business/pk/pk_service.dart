...
class PKService implements PKServiceInterface {
  ...
  Future<PKInviteSentResult> invitePKbattle(
      List<String> targetUserIDList, bool autoAccept) async {
    if (pkInfo != null) {
      final result =
      ...
      pkExtendedMap['user_name'] = localUser?.userName ?? 'A Tussle User';
      pkExtendedMap['user_avatar_url'] = localUser?.avatarUrlNotifier.value;

      pkStateNoti.value = RoomPKState.isRequestPK;
      final result = await sendUserRequest(
        targetUserIDList,
        jsonEncode(pkExtendedMap),
        true,
      ).catchError((error) {
  ...
  void updatePKRoomAttributes() {
    ...
    if (ZegoLiveStreamingManager().hostNoti.value != null) {
      pkMap['host_user_id'] =
          ZegoLiveStreamingManager().hostNoti.value?.userID ?? '';
    }
    ...
    ZEGOSDKManager().zimService.setRoomAttributes(
          pkMap,
